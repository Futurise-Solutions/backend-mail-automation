...
- Signature to use or reference: "${settings.companySignature.replace(/<[^>]*>/g, '')}"
...
7. Format the output as a clean HTML email (use standard paragraph tags, bold text where appropriate, etc.) along with a plain-text backup.

You MUST respond strictly in the following JSON format:
{
  "subject": "Email Subject Line Here",
  "html": "<p>HTML-formatted email body. Add line breaks using tags. Do not include external CSS. Include signature.</p>",
  "text": "Plain-text version of the email, including signature."
}
`;
...
6. Use our company signature: "${settings.companySignature.replace(/<[^>]*>/g, '')}"
7. Must respond in standard JSON format:

{
  "subject": "Re: ${previousEmailContent.subject.replace(/^Re:\s*/i, '')}",
  "html": "<p>HTML-formatted email follow-up body. Include signature.</p>",
  "text": "Plain-text version of the follow-up, including signature."
}
`;
...
Signature to use:
"${settings.companySignature.replace(/<[^>]*>/g, '')}"
